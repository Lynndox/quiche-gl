...
/// A wrapper for some data to aligns the inner `T` to 4 bytes.
///
/// This is important for data sent to the QPU, as it _must_ be aligned to 4
/// byte boundaries.
#[repr(align(4))]
pub struct Align4<T> {
    inner: T,
...
impl<T> Copy for Align4<T> where T: Copy {}

impl<T> Align4<T> {
    /// Wrap [`T`] to align it to a 4 byte boundary.
    pub const fn new(inner: T) -> Self {
        Self { inner }
    }
