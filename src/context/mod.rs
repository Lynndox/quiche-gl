...
pub struct Context<S, M: MemoryMapper = IdentityMapper> {
    display: Display,
    mapper: M,
    state: S,
}
...
impl<S, M: MemoryMapper> Context<S, M> {
    ...
    {
        let mailbox =
            unsafe { &*(self.mapper.phys_to_virt_addr(MAIL_BASE as _)? as *const RawMailbox) };

        let msg_phys_addr = self.mapper.phys_to_virt_addr(&raw const *message as _)?;
        let msg_send_addr = (msg_phys_addr as u32 & !0xF) | message.channel() as u32;
    ...
    crate::Error: From<<M as MemoryMapper>::Error>,
{
    ...
    pub fn initialize(mut self, double_buffer: bool) -> Result<Context<Initialized, M>> {
        unsafe {
            ...
            let buf_ptr = self.mapper.phys_to_virt_addr(bus_to_arm_addr(buf_ptr))?;
